...
class SoftActorCritic(object):
    ...
    def select_action(self, state, eval=False):
        return self.policy_net.get_action(state, eval)

    def update_parameters(self, memory, batch_size, updates,
                            gamma       = 0.99,
                            soft_tau    = 0.01
                      ):
        state, action, reward, next_state, mask = memory # mask = (1-done)

        state      = torch.FloatTensor(state).to(self.device)
        next_state = torch.FloatTensor(next_state).to(self.device)
        action     = torch.FloatTensor(action).to(self.device)
        reward     = torch.FloatTensor(reward).unsqueeze(1).to(self.device)
        mask       = torch.FloatTensor(np.float32(mask)).unsqueeze(1).to(self.device)
        ...
        target_value = reward \
                        + mask * gamma * (torch.min(target_q2_value, target_q2_value) \
                                    - ent_coef * log_prob)
        ...
        return q_value_loss.item(),policy_loss.item(),ent_loss.item()
